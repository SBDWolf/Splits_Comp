...
try:
    ...
    #new function here, gets called after generating a splits list. if user consents, it updates the xml file.
    #livesplit's lss files keep comparisons inside <SplitTimes /> tags for every segment, so we're just adding child elements to those.
    def generate_splits_file(generated_segment_list, comparison_name):
        curr_arr_position = 0
        for split in root.iter('SplitTimes'):
            new_comparison = ET.Element("SplitTime")
            new_comparison.set('name', comparison_name)
            realtime = ET.Element("RealTime")
            realtime.text = generated_segment_list[curr_arr_position]
            curr_arr_position += 1
            new_comparison.append(realtime)
            split.append(new_comparison)
        tree = ET.ElementTree(root)
        ET.indent(tree)
        tree.write(sys.argv[1], encoding='UTF-8', xml_declaration=True)
        print("The comparison has been added to the splits file.")
    ...
    def find_goal_splits(goal):
        ...
        #creating an array of the splits in string format so that it can be passed to generate_splits_file and create elements for the resulting xml file
        generated_segment_list = []
        for splits in segments[:-1]:
            time += time_at_percentile(splits, percentile)
            generated_segment_list.append(str(time))
            print(time)
        generated_segment_list.append(str(goal))
        print(goal)
        comparison_name = 'Balanced Target'
        generate_splits_file(generated_segment_list, comparison_name)        
    ...
    def find_reset_splits(goal):
        ...
        while iterations < max_iterations:
            ...
            #creating a copy of the array times in string format so that it can be passed to generate_splits_file and create elements for the resulting xml file
            generated_segment_list = []
            for time in times:
                print(time)
                generated_segment_list.append(str(time))
